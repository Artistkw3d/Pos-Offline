/**
 * Database Schema - Shared table definitions
 * Single source of truth for all 33 tables
 */
...
function createAllTables(db) {
  db.exec(TABLES_SQL);
  try { db.exec(INDEXES_SQL); } catch (_e) { /* ignore */ }
}

/**
 * Insert default settings
 * @param {Database} db - better-sqlite3 Database instance
 * @param {boolean} allSettings - if true, insert all settings; if false, only loyalty settings
 */
function insertDefaultSettings(db, allSettings) {
  const settings = allSettings ? DEFAULT_SETTINGS : DEFAULT_SETTINGS.slice(0, 3);
 ...
 * @param {Database} db - better-sqlite3 Database instance
 */
function insertDefaultBranch(db) {
